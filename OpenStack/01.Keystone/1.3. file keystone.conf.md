...
Khi lên cấu hình một hệ thống. Việc sử dụng file INI luôn được xem xét tới. Ini rõ ràng tốt hơn sử dụng XML trong các trường hợp:

- Dữ liệu không đòi hỏi Unicode.
- Dữ liệu không đòi hỏi chuẩn hóa, sử dụng rỗng rãi. Đơn thuần chỉ áp dụng cho cấu hình phần mềm.
- Dữ liệu đơn giản, không chia thành nhiều lớp phức tạp.
...
- `boolean` : Chứa giá trị `true` và `false`.
...
Các tùy chọn cài đặt được nhóm lại thành các section. Thông thường hầu hết các file config của OpenStack đều có 2 section [DEFAULT] và [database]. Section 
...
## Các section trong file keystone.conf
Nội dung file được tham khảo tại [đây](https://docs.openstack.org/keystone/latest/configuration/samples/keystone-conf.html)
